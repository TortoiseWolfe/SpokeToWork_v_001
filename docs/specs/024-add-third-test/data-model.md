...
- Email: `TEST_USER_PRIMARY_EMAIL` from `.env`
- Password: `TEST_USER_PRIMARY_PASSWORD` from `.env`
...
- Test passwords must be strong (12+ chars, mixed case, numbers, symbols)
- Stored in `.env` file (NOT committed to repo)
- `.env.example` shows format only (no real passwords)
- See `CLAUDE.md: Test Users` section for credential requirements
